// 2024-11-22 평균값 구하기 useMemo 사용하기 및 useCallback 사용하기 및 useRef 사용하기
...
const Average4 = () => {
  ...
  // useCallback(생성하고싶은 함수, 배열)
  const onChange = useCallback(e => {
    setNumber(e.target.value);
    // 입력 필드의 값을 실시간으로 가져와 setNumber로 인해 number 상태에 저장합니다.
  }, []);
  // 컴포넌트가 처음 랜더링 될때만 함수를 생성, 두번째 인자값에 빈 배열을 할당하면 리 랜더링 될때에는 미리 생성된 함수를 재사용
  ...
  const onInsert = useCallback(() => {
    ...
    if (isNaN(parsedNumber)) {
      ...
      setNumber(''); // 입력 필드 초기화
      inputNumber.current.focus(); // 입력필드 포커스 이동
      return; // 함수 종료
    }
  ...
  }, [number, list]); // number혹은 list가 변경될 때만 함수를 실행한다.
  ...
  return (
    <div>
      평균을 구할 정수 숫자 입력 (Average4 컴포넌트) :
      <input name="number" id="number" value={number} size="14" onChange={onChange} onKeyDown={onKeyPress} ref={inputNumber} />&nbsp;
